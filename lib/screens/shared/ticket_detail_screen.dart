...
class TicketDetailScreen extends StatefulWidget {
  final Ticket ticket;
  final VoidCallback? onBack; // Callback baru untuk kembali ke list di dalam tab
...
class _TicketDetailScreenState extends State<TicketDetailScreen> {
  ...
  void _updateStatus(TicketStatus newStatus) {
    ...
    if (index != -1) DummyData.tickets[index] = updatedTicket;
    setState(() => _currentTicket = updatedTicket);
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Status ${_currentTicket.id} berhasil diubah')));
  }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(child: Text('Tracking / Riwayat', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold))),
                if (isStaff)
                  TextButton.icon(
                    onPressed: () => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Fitur Assign Tiket'))),
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text('${history.userName} • ${history.timestamp.toString().substring(0, 16)}', style: const TextStyle(color: Colors.grey, fontSize: 12)),
                          ],
                        ),
                      ),
                    ],
